...
class DjangoAPI {
    ...
    async update_library(libraryId, updates) {
        await this.wrappedJsonFetch(
            'updating library',
            this.urls.api.library(libraryId),
            {
                method: 'patch',
    ...
    async add_item_to_library(libraryId, data) {
        await this.wrappedJsonFetch(
            'adding item to library',
            this.urls.api.libraryItems(libraryId),
            {
                method: 'post',
                body: JSON.stringify(data),
            },
    ...
    async list_users() {
        const response = await this.wrappedFetch('listing users', this.urls.api.users(), {
            method: 'get',
        });
        return response.json();
    }
