...
$('#openbem').on('change keyup','#region-postcodes-modal #postcode', function(){
    ...
    datasets.table_U6_region_from_postcode.forEach(function(element){
        // element[0] is the postcode
        if (element[0].indexOf(postcode.toUpperCase()) == 0) {
            $('#regions-by-postcode').append('<p style="padding-left:15px"><input type="radio" name="region" value="'+element[1]+'"> ' + element[0] +' - ' + datasets.regions[element[1]]+'</p>');
        }// element[1] is the region index
    });
});
