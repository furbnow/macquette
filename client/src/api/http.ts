...
export class HTTPClient {
  ...
  constructor(nodeOptions?: { sessionId: string; csrfToken?: string; baseURL: string }) {
  ...
  async listOrganisations(): Promise<Result<Organisation[], HttpClientError>> {
    return (
      ...
      if (parsed.success === false) {
        return Result.err(new HttpClientError('listing organisations', parsed.error));
      }
  ...
  > {
    ...
    return (
      ...
      (
        ...
        AddressSuggestionResponse['error'] | HttpClientError
      > => {
        const parsed = addressSuggestionResponse.safeParse(camelise(response.data));
        if (parsed.success === false) {
  ...
  > {
    return (
      ...
      (
        ...
        if (parsed.success === false) {
          return Result.err(new HttpClientError('resolving address', parsed.error));
        }
