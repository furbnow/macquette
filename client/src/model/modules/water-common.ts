...
export function extractWaterCommonInputFromLegacy(scenario: Scenario): WaterCommonInput {
  const { low_water_use_design, annual_energy_content, override_annual_energy_content } =
  ...
  const solarHotWater =
    isTruthy(scenario?.use_SHW) || isTruthy(scenario?.water_heating?.solar_water_heating);
  return {
...
export type WaterCommonDependencies = {
  occupancy: {
    occupancy: number;
  };
};
...
export class WaterCommon {
  ...
  get hotWaterEnergyContentAnnual() {
    if (this.input.annualEnergyContentOverride !== false) {
      ...
      return sum(Month.all.map((month) => this.hotWaterEnergyContentByMonth(month)));
    }
  }
