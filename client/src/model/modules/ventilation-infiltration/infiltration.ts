...
export function extractInfiltrationInputFromLegacy(
  ...
  if (ventilation?.air_permeability_test === true) {
    estimateFrom = {
      type: 'pressure test',
      airPermeability: coalesceEmptyString(ventilation?.air_permeability_value, 0) ?? 0,
    };
  } else {
    ...
    estimateFrom = {
      ...
      draughtProofedProportion:
        coalesceEmptyString(ventilation?.percentage_draught_proofed ?? 0, 0) / 100,
...
export class Infiltration {
    ...
    switch (this.input.estimateFrom.type) {
      case 'pressure test': {
        ...
        const correctionFactor =
          this.dependencies.fabric.envelopeArea / this.dependencies.floors.totalVolume;
  ...
  private get rawAirChangesPerHour(): number {
    return this.structuralAirChangesPerHour + this.intentionalVentsFluesAirChangesPerHour;
  }
  ...
  mutateLegacyData(data: any) {
    ...
    switch (this.input.estimateFrom.type) {
      ...
      case 'pressure test': {
        ventilation.structural_infiltration_from_test = this.structuralAirChangesPerHour;
        break;
      }
    }
    ...
    ventilation.infiltration_rate_incorp_shelter_factor =
      this.withShelterFactorAirChangesPerHour;
    ventilation.adjusted_infiltration = Month.all.map((m) => this.airChangesPerHour(m));
