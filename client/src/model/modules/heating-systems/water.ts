...
export class WaterHeatingSystem {
    ...
    primaryCircuitLossMonthly(month: Month) {
        ...
        if (
            this.input.primaryCircuitLoss === null ||
            pipeworkInsulatedFraction === null
        ) {
            return 0;
        }
        ...
        const basicLoss =
            month.days *
            14 *
            ((0.0091 * pipeworkInsulatedFraction +
                0.0245 * (1 - pipeworkInsulatedFraction)) *
                heatingHoursPerDay +
                0.0263);
...
export function extractWaterHeatingSystemInput(
    ...
    if (
        !(
            legacySystem.provides === 'heating_and_water' ||
            legacySystem.provides === 'water'
        )
    ) {
        return null;
    }
