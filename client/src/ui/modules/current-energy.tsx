        ...
        <tbody>
          <tr>
            <th style={{ fontWeight: 'normal' }}>
              ...
              <InfoTooltip>
                The total energy used in the household, including the on-site portion of
                generated energy.
              </InfoTooltip>
            </th>
            <td>
              <NumberOutput value={currentEnergy.annualEnergyEndUse} dp={0} unit="kWh" />
            </td>
          </tr>
          <tr>
            <th style={{ fontWeight: 'normal' }}>
              <span style={{ fontWeight: 'bold' }}>Primary energy consumption</span>
              <InfoTooltip>
                All primary energy from all energy sources, not including generation.
              </InfoTooltip>
            </th>
            <td>
              <NumberOutput value={currentEnergy.annualPrimaryEnergy} dp={0} unit="kWh" />
            </td>
          </tr>
          <tr>
            ...
            <td>
              <NumberOutput
                value={currentEnergy.annualPrimaryEnergy / floors.totalFloorArea}
                dp={0}
                unit="kWh/m²"
              />
            </td>
          ...
          <tr>
            <th style={{ fontWeight: 'normal' }}>
              ...
              <InfoTooltip>
                All CO₂ emitted from all energy sources, not including generation (since
                generation is assumed to be zero-carbon).
              </InfoTooltip>
            </th>
            <td>
              <NumberOutput
          ...
          <tr>
            ...
            <td>
              <NumberOutput
                value={currentEnergy.annualCarbonEmissions / floors.totalFloorArea}
                dp={0}
...
function AddFuelModal({
  ...
  const availableFuels = filterValues(
    fuels,
    (_, fuel) => fuel.category !== 'Generation' && !fuelNamesInUse.includes(fuel.name),
  );
        ...
        {Object.keys(annualEnergyConsumptionByFuel).length === 0 ? (
          ...
          Object.entries(annualEnergyConsumptionByFuel).map(
            ([fuelName, annualEnergy]) => {
              const modelOutputFuel = currentEnergy?.fuels[fuelName] ?? null;
              return (
                <tr key={fuelName}>
                  <MiddleAlignedCell>
                    <label htmlFor={`consumption-row-${fuelName}`}>{fuelName}</label>
                  </MiddleAlignedCell>
                  ...
                  <MiddleAlignedCell className="align-right text-tabular-nums">
                    × <NumberOutput value={modelOutputFuel?.fuel.carbonEmissionsFactor} />
                  </MiddleAlignedCell>
                  <MiddleAlignedCell className="align-right text-tabular-nums">
                    <NumberOutput
                  ...
                  <MiddleAlignedCell className="align-right text-tabular-nums">
                    × <NumberOutput value={modelOutputFuel?.fuel.primaryEnergyFactor} />
                  </MiddleAlignedCell>
                  <MiddleAlignedCell className="align-right text-tabular-nums text-nowrap">
                    <NumberOutput
                  ...
                  <MiddleAlignedCell className="align-right text-tabular-nums text-nowrap">
                    <NumberOutput value={modelOutputFuel?.fuel.unitPrice} unit="p/kWh" />
                  </MiddleAlignedCell>
                  <MiddleAlignedCell className="align-right text-tabular-nums">
                    £
                    <NumberOutput value={modelOutputFuel?.fuel.standingCharge} dp={2} />
                  </MiddleAlignedCell>
                  <MiddleAlignedCell className="align-right text-tabular-nums">
                    £
                    <NumberOutput value={modelOutputFuel?.annualCost} />
                  </MiddleAlignedCell>
                  <MiddleAlignedCell>
                    <button
                      className="btn"
      ...
      <table className="table mb-15" style={{ width: 'auto' }}>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td rowSpan={2}>
              Calorific value of mains gas varies over time and by region. You can check
              against current figures, or use this as an approximation.
            </td>
          </tr>
          <tr>
            <td>1ft³</td>
    ...
    <table className="table mb-15">
      ...
      <tbody>
        <tr>
          <MiddleAlignedCell className="align-right text-tabular-nums">
            ×{' '}
            <NumberOutput value={currentEnergy?.generation?.fuel.carbonEmissionsFactor} />
          </MiddleAlignedCell>
          <MiddleAlignedCell className="align-right text-tabular-nums">
            <NumberOutput
          ...
          <MiddleAlignedCell className="align-right text-tabular-nums">
            × <NumberOutput value={currentEnergy?.generation?.fuel.primaryEnergyFactor} />
          </MiddleAlignedCell>
          <MiddleAlignedCell className="align-right text-tabular-nums text-nowrap">
            <NumberOutput
          ...
          <MiddleAlignedCell className="align-right text-tabular-nums">
            £
            <NumberOutput value={currentEnergy?.generation?.annualCostSaved} />
          </MiddleAlignedCell>
        </tr>
      </tbody>
    </table>
...
function Generation({
  ...
  return (
    <>
      <FormGrid>
        ...
        {generation.type === 'onsite' && (
          <>
            ...
            <label htmlFor="generation-fraction-onsite">Fraction used onsite</label>
            <NumberInput
      ...
      {generation.type === 'onsite' && <GenerationTable modelOutput={modelOutput} />}
    </>
  );
}

function extractStateFromLegacy(scenario: Scenario): Partial<State> {
  const { kwhdpp, kgco2perm2, primary_energy_use_m2, currentenergy, fuels, energy_use } =
    scenario ?? {};
  return {
    totals: {
      baseline: {
    ...
    annualEnergyConsumptionByFuel: mapValues(currentenergy?.use_by_fuel ?? {}, (fuel) =>
      coalesceEmptyString(fuel.annual_use, null),
    ),
    ...
    generation: {
      ...
      onSite: {
        annualEnergy:
          coalesceEmptyString(currentenergy?.generation?.annual_generation, null) ?? null,
        fractionUsedOnsite:
          coalesceEmptyString(currentenergy?.generation?.fraction_used_onsite, null) ??
          null,
        fitAnnualIncome:
          coalesceEmptyString(currentenergy?.generation?.annual_FIT_income, null) ?? null,
      },
    },
  };
}
...
export const currentEnergyModule: UiModule<State, Action, never> = {
  ...
  reducer: (state, action) => {
    switch (action.type) {
      ...
      case 'current energy/update fuel use': {
        state.annualEnergyConsumptionByFuel[action.fuelName] = action.annualEnergy;
        return [state];
      }
      ...
      <>
        <section className="line-top mb-45">
          <div className="mb-15">
            ...
            <p>
              The figures shown below on the left are the totals for actual energy use. If
              the input data is accurate, these figures represent real values.
            </p>
            <p>
              The graphs compare these values against the ones given by the model for the
              baseline scenario. According to the characteristics of the building, target
              temperature, type of fuel, etc. the model calculates the theoretical amount
              of energy needed to run the dwelling.
            </p>
            <p>
          ...
          <div className="d-flex justify-content-between mb-30">
            ...
            <TargetBars totals={state.totals} modelOutput={state.modelOutput} />
          </div>
        </section>
        <section className="line-top mb-45">
  ...
  shims: {
    ...
    mutateLegacyData: ({ project: projectRaw }, _, state) => {
      ...
      data.currentenergy.onsite_generation = state.generation.type !== 'none' ? 1 : false;
      data.currentenergy.generation = {
        ...
        fraction_used_onsite: state.generation.onSite.fractionUsedOnsite ?? undefined,
      };
