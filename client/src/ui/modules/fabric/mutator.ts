  ...
  return {
    ...
    ...(includeSpecifics === true
      ? {
          ...
          ...(wall.outputs.netArea !== null ? { netarea: wall.outputs.netArea } : null),
          ...(wall.outputs.windowArea !== null
            ? { windowarea: wall.outputs.windowArea }
            : null),
          ...(wall.outputs.heatLoss !== null ? { wk: wall.outputs.heatLoss } : null),
          ...(wall.type === 'measure'
            ? {
...
export function mutateLegacyData(
  ...
  for (const wall of state.walls) {
    ...
    if (legacyWallIdx === -1) {
      dataAny.fabric.elements.push(wallLikeToLegacy(wall, { includeSpecifics: true }));
    } else {
      ...
      const inBulkMeasure =
        state.bulkMeasures.findIndex((bm) => bm.appliesTo.includes(wall.id)) !== -1;
