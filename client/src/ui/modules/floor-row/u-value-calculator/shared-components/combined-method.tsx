      ...
      <table className={combinedMethodBem('table')}>
        <thead>
          <tr className={combinedMethodBem('heading-row')}>
            <th className={combinedMethodBem('layer-id-heading-cell')}>Layer</th>
            <th className={combinedMethodBem('thickness-heading-cell')}>Thickness</th>
            <th className={combinedMethodBem('main-material-heading-cell')}>
              Main material
            </th>
            <th className={combinedMethodBem('bridging-material-heading-cell')}>
              Bridging material
            </th>
            <th className={combinedMethodBem('bridging-proportion-heading-cell')}>
              Bridging proportion
              <InfoTooltip>
                <p>
                  The bridging portion of a suspended ground floor or exposed upper floor
                  is made up of the timber structure that passes through the insulation
                  layer. The amount of bridging can vary significantly, and sometimes
                  timbers are added to deal with structural problems or accommodate
                  alterations and hatches, increasing the amount of bridging.
                </p>
                <p>
                  In a standard floor with 70mm wide joists at 600mm centres it can be
                  assumed to be around 12%, or approximately 16% in a timber floor with
                  70mm joists at 450mm centres. You find this percentage by dividing the
                  width of the joist (usually 75-100mm) by the distance between centres
                  (usually 450-600mm). If this is unknown because you cannot access the
                  floor structure, then assume 600mm centres and 75mm joists.
                </p>
              </InfoTooltip>
            </th>
            <th></th>
        ...
        <tbody>
          {layers.map((layer, index) => (
            <Layer layer={layer} dispatch={dispatch} key={index} index={index} />
          ))}
          <tr
...
function Layer({ layer, dispatch, index }: LayerProps) {
  ...
  return (
    <tr className={combinedMethodBem('layer-row', index % 2 === 1 ? 'stripe' : null)}>
      <td className={combinedMethodBem('layer-id-cell')}>{(index + 1).toString(10)}</td>
      <td>
        <NumberInput
          ...
          onChange={(value) =>
            dispatch({
              ...
              payload: {
                bridging: {
                  proportion:
                    value === null ? null : Proportion.fromPercentClamped(value),
                },
              },
            })
          }
