...
type Action =
  ...
  | {
      type: 'merge state';
      toMerge: Partial<
        Pick<LoadedState, 'location' | 'area' | 'exposedPerimeter' | 'selectedFloorType'>
      >;
    };
...
export const floorRowModule: UiModule<LoadingState | LoadedState, Action, never> = {
  ...
  shims: {
    extractUpdateAction: ({ currentScenario, scenarioId, currentModel }, instanceKey) => {
      ...
      if (
        ...
        perFloorTypeSpec.custom.uValue = coalesceEmptyString(dataElement.uvalue, null);
      } else {
      ...
      const action: Action = {
        ...
        floorSpec: {
          ...
          exposedPerimeter: coalesceEmptyString(dataElement.perimeter, null) ?? 0,
          selectedFloorType,
          perFloorTypeSpec,
        },
  ...
  component: function FloorRow({ state, dispatch }) {
    const calculatorRowContent = state.loaded && state.selectedFloorType !== 'custom' && (
      <>
        <td></td>
        <td colSpan={7}>
          <FUVC
    ...
    return (
      <>
        ...
        <td>
          <Select
            ...
            options={(
              [
                'solid (bs13370)',
                ...(state.showLegacySolidFloorOption ? ['solid' as const] : []),
                'suspended',
                'exposed',
                'heated basement',
                'custom',
        ...
        <td>
          ...
          />
          <InfoTooltip>
            This field is only enabled when the exposed perimeter value is necessary for
            calculating the floor U-value; if it is disabled then the exposed perimeter is
            not required.
          </InfoTooltip>
        </td>
        <td>
