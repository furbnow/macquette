...
export function extractFloorDataElement(
  ...
  const element = scenario?.fabric?.elements?.find((elem) => elem.id === parsedElementId);
...
export function extractRawFloorElement(
  ...
  if (element === undefined) {
    throw new Error(`Could not find a fabric element with the provided ID: ${elementId}`);
  }
