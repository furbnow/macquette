...
export function TargetBar({
  ...
  function determineStatus() {
    const filteredValues = values.filter(
      (datapoint): datapoint is Exclude<typeof datapoint, null> => {
        if (datapoint === null || Number.isNaN(datapoint) || datapoint === Infinity) {
          return false;
        } else {
          return true;
        }
  ...
  const hasZeroTarget =
    targets !== undefined && targets.filter((target) => target.value === 0).length !== 0;
  ...
  return (
    <div className="targetbar">
      <div className="targetbar-head">
        <span>{name}</span>
        {status.hasData === false && <span className="text-bold">Not enough data</span>}
      </div>
      ...
      >
        ...
        {/* Vertical axis line */}
        <line x1={1.5} y1={1.5} x2={1.5} y2={barsHeight + 0.5} stroke={axisColour} />
            ...
            return (
              <g key={idx} clipPath={`url(#clip-target-${id}-${idx})`}>
                <line
                  ...
                  stroke={axisColour}
                />
                <text x={x + 5} y={y + height - 22} style={{ fill: 'var(--grey-300)' }}>
                  {target.value} {units}
                </text>
                <text
                  x={x + 5}
