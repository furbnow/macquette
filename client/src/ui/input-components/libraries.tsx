  ...
  const filtered = libraries.data
    ...
    .map((library: FabricElementsLibrary) => ({
      ...
      data: pickBy<FabricElement, Wall | Roof | Loft | PartyWall>(
        library.data,
        discriminateTags<FabricElement, Wall | Roof | Loft | PartyWall, typeof tagType>(
          tagType,
        ),
      ),
    }))
    .map((library) => ({
      ...
      data: mapValues(library.data, (element, tag) => ({
        ...
        tag,
        uvalue:
          element.uvalue === '' ? fail(`bad uvalue: ${element.uvalue}`) : element.uvalue,
        kvalue:
          element.kvalue === '' ? fail(`bad kvalue: ${element.kvalue}`) : element.kvalue,
        type,
      })),
    }));
          ...
          ? [
              {
                ...
                value: (row: CompleteWallMeasure) => (
                  <NumberOutput
                    value={Math.round(
                      row.min_cost + areaSqm * row.cost * (row.EWI ? 1.15 : 1),
                    )}
                  />
                ),
              },
