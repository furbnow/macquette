...
import {
  exportsForTest,
  TabularFunction,
} from '../../../src/model/datasets/tabular-function';
...
describe('tabular function', () => {
    ...
    test('returns err when given an empty array', () => {
      fc.assert(
        fc.property(arbFloat(), (needle) => {
          expect(findClosestLeftIndex(needle, []).unwrapErr()).toBe('empty input');
        }),
      );
    });
  ...
  describe('simple linear interpolation', () => {
    ...
    it('does interpolation', () => {
      expect(linearInterpolateClamped({ x0: 0, y0: 100, x1: 5, y1: -400 }, 1)).toBe(0);
      expect(linearInterpolateClamped({ x0: -0.5, y0: 20, x1: 0, y1: 40 }, -0.1)).toBe(
        36,
      );
    });

    it('does edge cases', () => {
      expect(linearInterpolateClamped({ x0: 0, y0: 100, x1: 5, y1: -400 }, 0)).toBe(100);
      expect(linearInterpolateClamped({ x0: 0, y0: 100, x1: 5, y1: -400 }, 5)).toBe(-400);
    });
  });
});
