...
export function checkInputBugs(inputs: z.input<typeof scenarioSchema>) {
  function spaceHeatingEnergyRequirementsBugs(): Messages {
    if (
      ...
      /*
            ...
            Got that?
            */
      return bug('.space_heating was undefined but heating systems were specified', {});
    } else {
      return noBug;
    }
  }
  ...
  function solarHotWaterFlagBug(): Messages {
    ...
    const isBug =
      isTruthy(inputs?.use_SHW) !== isTruthy(inputs?.water_heating?.solar_water_heating);
    return isBug
  ...
  function ventilationSpecificFanPowerBug(): Messages {
    if (
      ...
      return bug('ventilation system must specify specific fan power, but it did not', {
