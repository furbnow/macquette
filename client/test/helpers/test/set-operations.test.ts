    ...
    test('onlyLHS, intersection and onlyRHS are all disjoint', () => {
      ...
      fc.assert(
        fc.property(arb, ({ lhs, rhs }) => {
          ...
          expect(compareSets(result.onlyLHS, result.intersection).intersection).toEqual(
            new Set(),
          );
          expect(compareSets(result.onlyLHS, result.onlyRHS).intersection).toEqual(
            new Set(),
          );
          expect(compareSets(result.intersection, result.onlyRHS).intersection).toEqual(
            new Set(),
          );
        }),
      );
