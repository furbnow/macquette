...
const knownBuggy: Array<{ reason: string; scenarios: Array<[string, string]> }> = [
  ...
  {
    reason:
      'heating system specifies a combi boiler with a primary circuit (which is not valid)',
    scenarios: [
  ...
  {
    reason:
      'some fuel contains a stringy value for standingcharge, which causes string concatenation bugs',
    scenarios: [
...
export function shouldSkipScenarioForGoldenMasterTest(
  ...
  for (const [path, name, reason] of knownBuggyFlat) {
    if (scenario.fixturePath === path && scenario.scenarioName === name) {
      console.warn(`Skipping known buggy scenario ${scenario.displayName} (${reason})`);
      return true;
    }
  }
